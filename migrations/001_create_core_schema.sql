...
-- EVENTS Policies
CREATE POLICY IF NOT EXISTS "events_select_all" ON public.events FOR SELECT USING (true);
CREATE POLICY IF NOT EXISTS "events_insert_authenticated" ON public.events FOR INSERT 
  WITH CHECK (auth.role() = 'authenticated' AND creator_id = auth.uid());
CREATE POLICY IF NOT EXISTS "events_update_creator" ON public.events FOR UPDATE 
  USING (auth.uid() = creator_id)
  WITH CHECK (auth.uid() = creator_id);
CREATE POLICY IF NOT EXISTS "events_delete_creator" ON public.events FOR DELETE 
  USING (auth.uid() = creator_id);

-- JOIN_REQUESTS Policies
CREATE POLICY IF NOT EXISTS "join_requests_select_own" ON public.join_requests FOR SELECT 
  USING (auth.uid() = user_id OR 
         EXISTS(SELECT 1 FROM public.events WHERE id = event_id AND creator_id = auth.uid()));
CREATE POLICY IF NOT EXISTS "join_requests_insert_own" ON public.join_requests FOR INSERT 
  WITH CHECK (auth.uid() = user_id);
CREATE POLICY IF NOT EXISTS "join_requests_update_creator" ON public.join_requests FOR UPDATE 
...
-- MESSAGES Policies
CREATE POLICY IF NOT EXISTS "messages_select_approved" ON public.messages FOR SELECT 
...
CREATE POLICY IF NOT EXISTS "messages_insert_approved" ON public.messages FOR INSERT 
...
-- RATINGS Policies
CREATE POLICY IF NOT EXISTS "ratings_select_all" ON public.ratings FOR SELECT USING (true);
CREATE POLICY IF NOT EXISTS "ratings_insert_participated" ON public.ratings FOR INSERT 
  WITH CHECK (auth.uid() = from_user_id AND
              ...
              AND status = 'approved'));
CREATE POLICY IF NOT EXISTS "ratings_update_own" ON public.ratings FOR UPDATE 
  USING (auth.uid() = from_user_id)
  WITH CHECK (auth.uid() = from_user_id);

-- CREW_REQUESTS Policies
CREATE POLICY IF NOT EXISTS "crew_requests_select_own" ON public.crew_requests FOR SELECT 
  USING (auth.uid() = from_user_id OR auth.uid() = to_user_id);
CREATE POLICY IF NOT EXISTS "crew_requests_insert_own" ON public.crew_requests FOR INSERT 
  WITH CHECK (auth.uid() = from_user_id);
CREATE POLICY IF NOT EXISTS "crew_requests_update_recipient" ON public.crew_requests FOR UPDATE 
...
CREATE TRIGGER IF NOT EXISTS update_join_requests_updated_at BEFORE UPDATE ON public.join_requests
  FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();

CREATE TRIGGER IF NOT EXISTS update_messages_updated_at BEFORE UPDATE ON public.messages
  FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();

CREATE TRIGGER IF NOT EXISTS update_crew_requests_updated_at BEFORE UPDATE ON public.crew_requests
  FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
