...
export function EventCard({ event }: EventCardProps) {
  ...
  const spotsLeft = event.maxPeople
    ? Math.max(event.maxPeople - attendeeCount, 0)
    : null;
  ...
  return (
    ...
    >
      ...
      >
          ...
          <View style={{ flex: 1 }}>
            ...
            {creator ? (
              <Text style={{ color: colors.muted, fontSize: 11 }}>
                {creator.age} ·{" "}
                {averageRating ? `${averageRating.toFixed(1)}★ karma` : "New"}
              </Text>
            ) : null}
          </View>
        </View>
        ...
        >
          <Text
            style={{ color: actionTextColor, fontWeight: "800", fontSize: 12 }}
          >
            {actionLabel}
          </Text>
        </Pressable>
